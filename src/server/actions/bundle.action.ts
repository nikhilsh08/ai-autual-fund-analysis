  ...
  try {
    ...
    const courses = await dataBasePrisma.course.findMany({
      where: {
        ...
        isPublished: true // Optional: ensure you only show published courses
      },
      select: {
        id: true,
        title: true,
    ...
    // 3. Return the bundle data combined with the full course objects
    return { 
      success: true, 
      data: { 
        ...bundle, 
        courses // This adds the array of full course objects to your response
      } 
    };
