  ...
  return (
    <div className="pt-28 pb-32 px-6 min-h-screen bg-cream">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl font-medium font-serif text-ink tracking-tighter mb-12">Your Cart</h1>

        {items.length === 0 ? (
          <div className="text-center py-24 bg-cream-dark rounded-2xl border border-border">
            <div className="w-16 h-16 bg-cream-darkest rounded-full flex items-center justify-center mx-auto mb-6 text-ink-muted">
              <ShoppingBag size={32} />
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2">
              <div className="flex justify-between items-center mb-6">
                <span className="text-sm text-ink-muted font-medium">{items.length} Course{items.length !== 1 && 's'}</span>
              ...
              <div className="space-y-4">
                {items.map((item) => (
                  <div key={item.id} className="flex gap-6 p-4 bg-card border border-border rounded-xl items-center shadow-sm">
                    <div className="relative w-24 h-24 rounded-lg bg-cream-dark flex-shrink-0 overflow-hidden">
                      {item.thumbnail ? (
                        <Image src={item.thumbnail} alt={item.title} fill className="object-cover" />
                      ) : (
                        <div className="w-full h-full bg-gradient-to-br from-accent-light to-cream-dark" />
                      )}
                    </div>
                    <div className="flex-1">
                      <div className="flex justify-between items-start">
                        <h3 className="text-lg font-medium text-ink">{item.title}</h3>
                        <button onClick={() => removeItem(item.id, isLoggedIn)} className="text-ink-muted hover:text-ink p-1">
                          <X size={20} />
                        </button>
                      </div>
                      <div className="mt-2 text-ink-secondary font-mono text-sm">₹{item.price}</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {upsellItem && !items.find((i) => i.id === upsellItem.id) && (
                <div className="mt-12 p-6 bg-cream-dark border border-border rounded-2xl relative overflow-hidden">
                  {/* Decorative Blob */}
                  ...
                  <div className="relative z-10 flex flex-col sm:flex-row justify-between gap-4 items-center">
                    <div className="flex items-center gap-4">
                      <div className="w-16 h-16 rounded-lg bg-gold/20 flex items-center justify-center text-gold overflow-hidden relative">
                    ...
                    <Button variant="secondary" onClick={handleAddUpsell} className="bg-card shadow-sm w-full sm:w-auto">
            ...
            <div className="lg:col-span-1">
              <div className="bg-cream-dark border border-border rounded-2xl p-6 sticky top-28 shadow-sm">
                <h3 className="font-medium text-ink mb-6">Order Summary</h3>
                <div className="space-y-4 text-sm mb-6">
                  <div className="flex justify-between text-ink-muted"><span>Subtotal</span><span className="font-mono">₹{subtotal.toFixed(2)}</span></div>
                  <div className="flex justify-between text-ink-muted"><span>GST (18%)</span><span className="font-mono">₹{tax.toFixed(2)}</span></div>
                ...
                <p className="text-center text-xs text-ink-muted mt-4">Secure checkout powered by Zwitch</p>
              </div>
            </div>
          </div>
        )}
